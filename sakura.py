...
async def userinfo(ctx,name=None):
    def rv(content):
        if isinstance(content,str):
            ...
            value.replace("`external_emojis`","`外部の絵文字の使用`").replace("`view_guild_insights`","`サーバーインサイトを見る`").replace("`connect`","`接続`")
    ...
    if member is not None:
        embed.add_field(name="ステータス",value=rv(member.status),inline=False)
    ...
    if member_:
        ...
        embed.add_field(name="ニックネーム",value=rv(member_.nick))
